...
WF = {
    ...
    "description": "Beatriz SDR agent on WhatsApp via WAHA with Postgres persistence — multi-tenant session routing, lead CRUD, qualification extraction, pipeline transitions, Chat Memory, and events log.",
    "nodes": [
        # 1. WAHA Webhook
        {
            "parameters": {
...
SELECT * FROM inserted;""",
        ...
        # 10. AI Agent
        {
            "parameters": {
                "promptType": "define",
                "text": "={{ $('WAHA Webhook').first().json.payload?.body || '' }}",
                "options": {
        ...
        # 11. Parse & Extract Fields (Code node)
        {
            "parameters": {
                "mode": "runOnceForAllItems",
...
WHERE $2 IS NOT NULL AND $2 != '';""",
        ...
        # 15. Webhook Response (200 OK — true branch)
        {
            "parameters": {
                "respondWith": "json",
